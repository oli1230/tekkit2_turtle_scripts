...
local function runCycle()
    ...
    for key, qty in pairs(totalToPush) do
        local itemId, damage = key:match("^(.+):(%d+)$")
        damage = tonumber(damage)
        if itemId and damage and qty > 0 then
            local slot = findSlot(itemId, damage)
            if slot then
                print("Pushing " .. qty .. "x " .. itemId)
                chest.pushItems(above_name, slot, qty)
            else
                print("Could not find " .. itemId .. " in chest!")
            end
        end
    end

    -- Monitor for returning items - check BEFORE dumping
    ...
    repeat
        ...
        if hasOutput then
            ...
            -- Only dump on a clear check to clean up any non-output items
            dumpTurtleInventory()
        end
    until clearCount >= 3
